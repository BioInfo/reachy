...
class LiteLLMBrain:
    ...
    def __init__(
        ...
        *,
        ...
        # Two reasoning knobs, because backends differ:
        #  reasoning_effort ("none"/"low"/"medium"/"high") — OpenAI / gpt-oss / Cerebras.
        #  reasoning_enabled (bool) — DeepSeek / OpenRouter `reasoning:{enabled}`.
        # reasoning_effort wins when set. Both keep reasoning tokens from eating the
        # max_tokens budget (-> empty reply) and adding latency on the voice path.
    ...
    def respond(self, text: str, history: list[Message]) -> Reply:
        ...
        try:
            ...
            if self.reasoning_effort:
                # gpt-oss / Cerebras / OpenAI style (top-level param via extra_body).
