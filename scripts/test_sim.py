...
Targets the simulation by default (port 8000, matching run_sim.sh and the port
installed apps connect to). Override with REACHY_HOST / REACHY_PORT env vars or
--host / --port flags so this never accidentally drives the physical robot
daemon (historically on :7860).

    ./scripts/run_test.sh                    # -> sim on :8000
...
parser.add_argument("--port", type=int, default=int(os.environ.get("REACHY_PORT", "8000")))
